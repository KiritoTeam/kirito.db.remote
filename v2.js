...
class KiritoDB {
  ...
  //Métodos da class do KiritoDB
    ...
    try {
      let pingt = Date.now();
      const api = await axios.get(`${Urlbase}/api/v2/db/add?id=ping&rota=ping&valor=1`);
      let pings = Date.now();
  ...
  try {
    ...
    const api = await axios.get(`${Urlbase}/api/v2/db/add?id=${this.requiredString}&rota=${rota}&valor=${valor}`);
  ...
  async get(rota) {
    try {
      ...
      const api = await axios.get(`${Urlbase}/api/v2/db/get?id=${this.requiredString}&rota=${rota}`);
  ...
  async sub(rota, valor) {
    try {
      ...
      const api = await axios.get(`${Urlbase}/api/v2/db/sub?id=${this.requiredString}&rota=${rota}&valor=${valor}`);
  ...
  async delete(rota) {
    try {
      ...
      const api = await axios.get(`${Urlbase}/api/v2/db/delete?id=${this.requiredString}&rota=${rota}`);
  ...
  async set(rota, valor) {
    try {
      ...
      const api = await axios.get(`${Urlbase}/api/v2/db/set?id=${this.requiredString}&rota=${rota}&valor=${valor}`);
  ...
  async all() {
    try {
      const api = await axios.get(`${Urlbase}/api/v2/db/all?id=${this.requiredString}`);
